...
console.log(singleProduct)
  ...
  if (!singleProduct) {
    return (
      <Container className={cn("mt-4 flex flex-col max-sm:pb-40 max-sm:px-4", className)}>
      ...
      <div className="flex justify-between max-sm:flex-col">
        ...
        <div className="w-[350px]">
          <p className="text-[32px] max-sm:text-[20px]">
            {singleProduct.name} {singleProduct.brand} {singleProduct.model} {singleProduct.memory}
          </p>
          ...
          {singleProduct.characteristic && (
            <div className="flex flex-col text-xl max-sm:text-sm max-sm:gap-3 gap-9">
              ...
              <p className="font-light">Экран: {singleProduct.characteristic.screen}</p>
              <p className="font-light">Количество ядер: {singleProduct.characteristic.cores}</p>
              <p className="font-light">Мощность блока питания: {singleProduct.characteristic.power}</p>
              <p className="font-light">Оперативная память (RAM): {singleProduct.characteristic.ram}</p>
              <p className="font-light">Встроенная память (ROM): {singleProduct.characteristic.rom}</p>
              <p className="font-light">Основная камера МПикс: {singleProduct.characteristic.camera}</p>
            </div>
          )}
        </div>
            ...
            {singleProduct.discount ? (
              <>
                <span className="font-bold text-[20px]">
                  {Math.round(singleProduct.price * (1 - (singleProduct.discount ?? 0) / 100))} грн
                </span>
              ...
              <span className="font-bold text-[20px]">{singleProduct.price} грн</span>
            )}
          </div>
